...
export const CashFlow = () => {
  return (
    <section id="payment" className="py-0 md:py-20 bg-white font-sans overflow-hidden border-b-2 border-green-100">
      ...
      {/* CONTAINER: px-0 mutlak di mobile agar tidak ada margin samping */}
      <div className="max-w-[1800px] mx-auto px-0 md:px-12 lg:px-20 relative z-10">
        
        {/* HEADER: Hanya teks yang diberi padding px-5 agar tidak menempel bezel */}
        <div className="max-w-4xl mb-8 md:mb-16 space-y-4 px-5 md:px-0 mt-10 md:mt-0">
          ...
          <p className="text-base md:text-xl text-slate-400 font-light leading-relaxed max-w-2xl border-l-4 border-green-500/20 pl-4 md:pl-6">
            Sistem pembayaran yang dapat disesuaikan dengan kebutuhan arus kas perusahaan Anda.
          </p>
        </div>

        {/* CONTENT GRID: Gap diset 0 di mobile */}
        <div className="grid lg:grid-cols-12 gap-0 md:gap-16 items-stretch">
          
          {/* SISI KIRI: Image & Badges */}
          <div className="lg:col-span-7 flex flex-col justify-between">
            {/* Image: rounded-none & shadow-none agar full-bleed sempurna */}
            <div className="relative aspect-video md:rounded-[2rem] overflow-hidden md:border-2 md:border-green-100 shadow-none md:shadow-xl group">
              ...
              {/* Badge di atas gambar: tetap ada padding kecil agar tidak menempel pojok layar */}
              <div className="absolute bottom-4 left-4 md:bottom-6 md:left-6 bg-white/95 backdrop-blur-md p-3 md:p-5 rounded-xl border-2 border-green-100 flex items-center gap-3">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-green-50 rounded-lg flex items-center justify-center text-green-600">
                  <BadgeCheck size={18} />
                </div>
                <div>
                  <p className="text-[8px] md:text-[9px] font-black text-green-700 uppercase tracking-widest leading-none mb-1">Audit Ready</p>
                  <p className="text-xs md:text-sm font-bold text-[#052c17]">Standard Korporasi</p>
                </div>
              </div>
            </div>

            {/* Checklist: px-5 untuk teks */}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 px-5 md:px-0 py-8 md:py-0 md:mt-8">
              {[
                ...
                <div key={i} className="flex gap-2 items-center">
                  <div className="flex-shrink-0 w-7 h-7 bg-green-50 rounded-lg flex items-center justify-center text-green-600">
                    <CheckCircle2 size={12}/>
                  </div>
                  <div className="min-w-0">
                    <h4 className="text-[#052c17] font-bold text-[10px] md:text-[11px] uppercase truncate">{item.title}</h4>
                    <p className="text-slate-400 text-[9px] md:text-[10px]">{item.sub}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="lg:col-span-5">
            {/* Box: rounded-none di mobile agar box hijau mentok ke sisi layar */}
            <div className="h-full bg-[#f7faf7] border-y-2 border-green-100 md:border-2 md:rounded-[2.5rem] p-5 md:p-10 space-y-8 md:space-y-10 shadow-none">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div 
                    key={idx}
                    className="group flex items-center justify-between p-4 bg-white border-2 border-green-50 rounded-xl transition-all shadow-sm"
                  >
                    <div className="flex items-center gap-4">
                      <div className="flex-shrink-0 p-2.5 bg-green-50 rounded-lg text-green-600">
                        {item.icon}
                      </div>
                      <div>
                        <p className="text-[#052c17] font-bold text-sm tracking-tight">{item.label}</p>
                        <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest">{item.desc}</p>
                      </div>
                    </div>
              ...
              <div className="pt-6 pb-6 md:pb-0">
                <p className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] text-center italic">
